...
from UE5_python_dependencies.net.handlers.stateless_connect import (
    StatelessConnectHandlerComponent,
    HandshakePacketType,
)
from UE5_python_dependencies.net.connection import NetConnection
from UE5_python_dependencies.net.error_reporter import PARSE_EXCEPTIONS
from UE5_python_dependencies.app_config import LOCAL_NETWORK_VERSION
...
class PcapAdaptor:
    ...
    def _decode_packet(self, conn: NetConnection, reader: FBitReader) -> dict:
        ...
        info["channels"] = ",".join(
            str(c) for c in sorted({b["ch_index"] for b in info["bunches"]})
        )
        return info
    ...
    def process_file(
        ...
        for _, pkt in captured:
            ...
            key = self._client_key(
                to_server, pkt[IP].src, sport if to_server else dport
            )
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(
        description="Decode UE5 packets from a .pcap using the UE5 network implementation."
    )
    p.add_argument(
        "input",
        nargs="*",
    ...
    for idx, f in enumerate(files, start=1):
        ...
        df.to_csv(
            out, mode="a", index=False, header=(idx == 1 or out.stat().st_size == 0)
        )
        if not bunch_df.empty:
            bunch_df.to_csv(
                ...
                index=False,
                header=(
                    idx == 1 or not bunch_out.exists() or bunch_out.stat().st_size == 0
                ),
            )
        del df, bunch_df
        ...
        print(
            ...
            f"{total['packets']} packets total so far)",
            flush=True,
        )
    ...
    print(
        f"Handshake packets: {total['handshake']}, data packets: {total['data']}, "
